...
| **Offline Reserve + optical QR protocol** | The distinctive component. Fountain-coded transport, device-key-signed requests/confirmations, hash-chained Reserve authorizations, replay + tamper rejection. Full two-device optical loop covered in-app; the redemption-on-reconnect leg is additionally captured on video (`demo/payflex_offline_demo.mp4`) running the production protocol code end-to-end with a **real Stellar testnet settlement** — hash `ad4b41bd…f5407`, confirmed via Horizon (ledger 4672758), memo `offline auth…4a1d`. See [`demo/README.md`](demo/README.md) for the honest production notes: every cryptographic step is the app's real code; only the transport is substituted (rendered frames + jsQR decode instead of photons between cameras). A literal two-physical-device recording remains on the pre-submission plan ([`demo/two-device-session-plan.md`](demo/two-device-session-plan.md)) |
