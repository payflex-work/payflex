...
/// Reusable five-step money-movement sheet: review, PIN confirmation,
/// submission, terminal result, and caller-led ledger refresh. Safebox uses
/// this instead of a separate payment path.
class PfPaymentConfirmationSheet extends StatefulWidget {
  final String title;
  ...
  const PfPaymentConfirmationSheet({super.key, required this.title, required this.recipientName, required this.purpose, required this.amount, required this.fee, required this.onAuthorize, required this.onSubmit, required this.onSuccess});

  @override
  State<PfPaymentConfirmationSheet> createState() => _PfPaymentConfirmationSheetState();
}

class _PfPaymentConfirmationSheetState extends State<PfPaymentConfirmationSheet> {
  ...
  Future<void> _continue() async {
    ...
    try {
      if (!await widget.onAuthorize(_pin.text)) {
        setState(() { _step = 1; _error = 'Enter your 4-digit transaction PIN.'; });
        return;
      }
      ...
      if (mounted) setState(() { _step = 1; _error = e.toString(); });
    }
  }
  ...
  Widget build(BuildContext context) => SafeArea(
    child: Padding(
      padding: EdgeInsets.fromLTRB(PfSpace.xl, PfSpace.lg, PfSpace.xl, MediaQuery.of(context).viewInsets.bottom + PfSpace.xl),
      child: _step == 3 ? _success() : Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(widget.title, style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: PfSpace.md),
          Text(_step == 0 ? 'Review' : _step == 1 ? 'Authenticate' : 'Submitting', style: Theme.of(context).textTheme.labelLarge),
          const SizedBox(height: PfSpace.lg),
          if (_step == 0) ...[
            _sheetRow('Destination', widget.recipientName),
            _sheetRow('Purpose', widget.purpose),
            _sheetRow('Amount', formatMoney(widget.amount.toString(), 'NGN')),
            _sheetRow('Fee', formatMoney(widget.fee.toString(), 'NGN')),
          ] else if (_step == 1) ...[
  ...
  Widget _success() => Column(mainAxisSize: MainAxisSize.min, children: [
    const Icon(Icons.check_circle_outline_rounded, color: PfColors.success, size: 52),
    const SizedBox(height: PfSpace.md),
    const Text('Payment submitted'),
    const SizedBox(height: PfSpace.sm),
    const Text('Your Safebox ledger has been updated.'),
    const SizedBox(height: PfSpace.lg),
  ...
  Widget _sheetRow(String label, String value) => Padding(
    ...
    child: Row(children: [Text(label, style: const TextStyle(color: PfColors.inkMuted)), const Spacer(), Flexible(child: Text(value, textAlign: TextAlign.right, style: const TextStyle(fontWeight: FontWeight.w600)))]),
  );
}
